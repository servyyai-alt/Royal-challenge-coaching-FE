...
export default function AdminLayout({ children, title }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-royal-950 text-white flex flex-col transition-transform duration-300 ${
      ...
      <div className="flex-1 lg:ml-64 min-h-screen flex flex-col">
        ...
        <header className="bg-white border-b border-gray-200 px-6 h-16 flex items-center justify-between sticky top-0 z-30 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-display font-bold text-royal-900 text-lg">
              {title}
            </h1>
          </div>
          <Link
            to="/"
            className="text-xs text-gray-500 hover:text-royal-800 transition-colors flex items-center gap-1"
          >
